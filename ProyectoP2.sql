...
DROP TRIGGER IF EXISTS VerificacionCompraInventario;
...
EXEC ModificarUsuario
    @ID_Jugador = 1,  -- Asegúrate de que este ID exista en tu tabla de prueba
    @NuevoNombre_Jugador = 'TestJugadorModificado',
    @NuevoCorreo_Electronico = 'modificado@correo.com',
...
EXEC ComprarMaterialPorNombre @Nombre_Material = 'Madera de roble', @ID_Jugador = 1, @Cantidad = 5;
...
select *from InventarioJugadores
select *from Materiales
SELECT name, collation_name FROM sys.columns WHERE object_id = OBJECT_ID('Jugadores');
...
drop trigger VerificacionVentaInventario
CREATE TRIGGER VerificacionVentaInventario
ON Ventas
AFTER INSERT
AS

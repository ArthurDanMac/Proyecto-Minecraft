...
-- Crear la funci�n que verifica la complejidad de la contrase�a con el uso de las tablas hash
...
CREATE TABLE Jugadores (
    ...
    ContraseñaHash VARBINARY(256), -- Almacenamiento de la contrase�a como hash
...
-- Crear un disparador para la modificaci�n en la tabla Compras
...
BEGIN
    ...
    ELSE
    BEGIN
        PRINT 'La cantidad deseada es mayor que la cantidad disponible. No se realiz� la compra.';
    END
END;
*/
DROP TRIGGER IF EXISTS VerificacionVentaInventario;
...
BEGIN
    ...
    BEGIN
        ...
        -- Resto del código para actualizar las tablas...
...
-- Continuación del ejemplo existente
...
-- Eliminar los datos existentes en InventarioJugadores para empezar de nuevo
DELETE FROM InventarioJugadores;
...
EXEC ObtenerMaterialesEnInventarioJugador @ID_Jugador=6;

EXEC ObtenerHerramientasEnInventarioJugador @ID_Jugador=2;
...
--procedimiento almacenado para crear un usuasio
...
BEGIN
    ...
    ELSE
        ...
        VALUES (@Nombre_Jugador, @Correo_Electronico, CAST(HASHBYTES('SHA2_256', @Contraseña) AS VARBINARY(256)), @Saldo_Esmeraldas);
END;

EXEC CrearUsuario 
    @Nombre_Jugador = 'TestJugador',
    @Correo_Electronico = 'test@correo.com',
    @Contraseña = 'Test@1234',
    @Saldo_Esmeraldas = 100;

--procedimiento almacenado para modificar al usuario 
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            UPDATE Jugadores SET ContraseñaHash = CAST(HASHBYTES('SHA2_256', @NuevaContraseña) AS VARBINARY(256)) WHERE ID_Jugador = @ID_Jugador;
    END
...
EXEC ModificarUsuario
    @ID_Jugador = 3,  -- Asegúrate de que este ID exista en tu tabla de prueba
    @NuevoNombre_Jugador = 'TestJugadorModificado',
    @NuevoCorreo_Electronico = 'modificadowewe@correo.com',
    @NuevaContraseña = 'Nueva@1234',
    @NuevoSaldo_Esmeraldas = 200;
--comprobacion
SELECT * FROM Jugadores;

exec ComprarMaterialPorNombre @Nombre_Jugador='Jugador2',@Nombre_Material=''
...
AS
BEGIN
    IF @Nombre_Jugador IS NULL OR @Nombre_Material IS NULL OR @Cantidad IS NULL
        RAISERROR('Ningún parámetro puede ser NULL.', 16, 1);

    DECLARE @ID_Jugador INT, @ID_Producto INT, @Precio INT;
    SELECT @ID_Jugador = ID_Jugador FROM Jugadores WHERE Nombre_Jugador = @Nombre_Jugador;
    IF @ID_Jugador IS NULL
        RAISERROR('Jugador no encontrado.', 16, 1);

    SELECT @ID_Producto = M.ID_Producto, @Precio = P.Precio_Esmeralda
    FROM Materiales M
    ...
    -- Inserta la compra
    INSERT INTO Compras (ID_Jugador, ID_Producto, Cantidad, PrecioTotal, FechaCompra)
    VALUES (@ID_Jugador, @ID_Producto, @Cantidad, @Precio * @Cantidad, GETDATE());
END;

drop trigger VerificacionCompraInventario
CREATE TRIGGER VerificacionCompraInventario
ON Compras
AFTER INSERT
AS
